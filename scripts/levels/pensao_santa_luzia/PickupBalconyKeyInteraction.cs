...
/// <summary>Sprint 17 — balcony key pickup (gated until note is read).</summary>
public partial class PickupBalconyKeyInteraction : Node, IInteractable
{
    ...
    public string GetPromptText()
    {
        ...
        return _state.HasReadBalconyNote ? "Pegar chave da varanda" : "Examinar";
    }
    ...
    public void Interact(Node interactor)
    {
        ...
        if (!_state.HasReadBalconyNote)
        {
            hud?.ShowMessage("Há várias coisas velhas aqui, mas nada chama atenção.", 3.0f);
            return;
        }

        _state.PickupBalconyKey();
        _pickedUp = true;
        hud?.ShowMessage("Encontrei uma chave pequena, tomada por ferrugem.", 3.0f);
        HidePickup();
    }
